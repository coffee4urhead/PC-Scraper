...
class ProBgScraper(AsyncPlaywrightBaseScraper):
    def __init__(self, website_currency, update_gui_callback=None):
        ...
        self.website_that_is_scraped = "Pro.bg"
    ...
    async def _extract_product_links(self, page, page_url):
        ...
        try:
            ...
            await page.wait_for_selector('div.grid-x.small-up-1', timeout=10000)

            product_elements = await page.query_selector_all('div.grid-x.small-up-1 div.cell')
            ...
            for product in product_elements:
                ...
                title_element = await product.query_selector('div.product_box h2')
                ...
                link_element = await product.query_selector('div.pv_img a[href]')
                if not link_element:
                    link_element = await product.query_selector('div.product_box a[href]')
                if not link_element:
                    link_element = await product.query_selector('h2 a[href]')
    ...
    async def _extract_product_data(self, page, product_url):
        ...
        try:
            ...
            await page.wait_for_selector('div.product_box_info h2', timeout=10000)

            title_element = await page.query_selector('div.product_box_info h2')
            title = await title_element.inner_text() if title_element else "N/A"
            if title_element:
                title = title.strip()
        
            price_container = await page.query_selector('div.cell.small-12.medium-12.large-3')
            if price_container:
                price_elements = await page.query_selector_all('div[style="color:#485059; font-size:18px; font-weight:700;"]')
            ...
            if price_text != "N/A":
                try:
                    price_str = price_text.split(" ")[0]
                    price = float(price_str)
                except Exception as e:
                    print(f"DEBUG: Price conversion error: {e} for text: {price_text}")
            ...
            tech_section = await page.query_selector('div.pi_desc')
            if tech_section:
                full_text = await tech_section.inner_text()
