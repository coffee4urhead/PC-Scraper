...
class AmazonDeScraper(AsyncPlaywrightBaseScraper):
    def __init__(self, website_currency, update_gui_callback=None):
        ...
        self.website_that_is_scraped = "Amazon.de"
        self.current_page = 1
    ...
    def _construct_page_url(self, base_url, search_term):
        if self.current_page > 1:
            return f"{self.base_url}s?k={quote(search_term)}&page={self.current_page}&xpid=L1vzNrANz4x19&crid=2J0PCSZU19ONB&qid=1760797196&sprefix={quote(search_term)}%2Caps%2C153&ref=sr_pg_2"
        return base_url
    ...
    async def _extract_product_links(self, page, page_url):
        ...
        try:
            ...
            await page.wait_for_selector('div[data-component-type="s-impression-counter"]', timeout=10000)
    ...
    async def _extract_price_from_page(self, page):
        """Extract price from Amazon DE page with multiple fallbacks"""
        ...
        if price_element_whole and price_element_fraction:
            ...
            whole_part = re.sub(r'[^\d]', '', whole_part)
            fraction_part = re.sub(r'[^\d]', '', fraction_part)
            ...
            if whole_part and fraction_part:
                ...
                try:
                    ...
                    print(f"DEBUG: Could not convert combined price: '{price_text}'")
        ...
        price_selectors = [
            ...
            'span[data-a-color="secondary"] .a-price-whole',  
        ]
        ...
        for selector in price_selectors:
            ...
            if price_element:
                ...
                try:
                    ...
                    if match:
                        german_price = match.group().replace('.', '').replace(',', '.')
                        return float(german_price)
